...
func DeleteaccesspointpolicyforobjectlambdaHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v20180820/accesspointforobjectlambda/%s/policy#x-amz-account-id", cfg.BaseURL, name)
...
func CreateDeleteaccesspointpolicyforobjectlambdaTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v20180820_accesspointforobjectlambda_name_policy#x-amz-account-id",
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("The name of the Object Lambda Access Point you want to delete the policy for.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteaccesspointpolicyforobjectlambdaHandler(cfg),
	}
}
